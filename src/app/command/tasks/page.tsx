    ...
    <PageShell>
      ...
      <MetricStrip min={140}>
        ...
        <MetricCard
          ...
          {...(counts.overdue > 0 ? { tint: "danger" as const } : {})}
        />
        <MetricCard label="Due this week" value={String(counts.due_this_week)} tone={counts.due_this_week > 0 ? "warn" : "neutral"} />
        <MetricCard label="Completed" value={String(counts.completed)} tone="ok" />
      </MetricStrip>
        ...
        <nav style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {TASK_VIEWS.map((v) => {
            ...
            return (
              <Link
                key={v}
                href={listHref("/command/tasks", { view: v, q: params.q })}
                style={{
                  padding: "6px 12px",
                  borderRadius: 999,
                  fontSize: 13,
      ...
      <Pagination
        ...
        total={total}
        prevHref={listHref("/command/tasks", { view, q: params.q, page: params.page - 1 })}
        nextHref={listHref("/command/tasks", { view, q: params.q, page: params.page + 1 })}
      />
    </PageShell>
  );
}
