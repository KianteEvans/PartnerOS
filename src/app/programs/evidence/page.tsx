...
export default async function EvidencePage({
  ...
  // Cross-phase: rank the evidence against the AWS catalog so the locker can point
  // at the best-fit program to pursue next (reuses the Program Fit loader).
  ...
  return (
    <PageShell>
      ...
      <Link href="/programs?view=fit" style={{ textDecoration: "none" }}>
        ...
        >
          <div style={{ display: "grid", gap: 3 }}>
            {fit.hasEvidence && fit.summary.bestNext ? (
              <>
                <strong style={{ fontSize: 14, color: "var(--text)" }}>
                  You&apos;re {fit.summary.bestNext.coveragePercent}% ready for {fit.summary.bestNext.name}
                </strong>
                <span style={{ color: "var(--muted)", fontSize: 13 }}>
                  {fit.summary.readyToPursue.length} ready to pursue · {fit.summary.totalGaps} requirement
                  {fit.summary.totalGaps === 1 ? " gap" : " gaps"} — see the full coverage analysis.
                </span>
              </>
            ) : (
              <>
      ...
      <Link href="/programs/evidence/case-studies" style={{ textDecoration: "none" }}>
        <Card
          interactive
          style={{
            display: "flex",
        ...
        >
          <div style={{ display: "grid", gap: 3 }}>
            ...
            <span style={{ color: "var(--muted)", fontSize: 13 }}>
              Reusable proof — drafts that count toward your program coverage and attach to Specialization
              applications.
            </span>
          </div>
          <span style={{ color: "var(--accent)", fontWeight: 600, fontSize: 13, whiteSpace: "nowrap" }}>
      ...
      <Panel
        ...
        actions={
          ...
          <a href="/programs/evidence/export" style={{ color: "var(--accent)", textDecoration: "none", fontSize: 13 }}>
            Export CSV
          </a>
        }
      >
        ...
        <nav style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {EVIDENCE_VIEWS.map((v) => {
            ...
            return (
              <Link
                key={v}
                href={listHref("/programs/evidence", { view: v, q: params.q })}
                style={{
                  padding: "6px 12px",
                  borderRadius: 999,
                  fontSize: 13,
      ...
      <Pagination
        ...
        total={total}
        prevHref={listHref("/programs/evidence", { view, q: params.q, page: params.page - 1 })}
        nextHref={listHref("/programs/evidence", { view, q: params.q, page: params.page + 1 })}
      />
    </PageShell>
  );
}
    ...
    <Card>
      ...
      <p style={{ fontSize: 13, margin: "0 0 10px" }}>
        {item.storageObjectId ? (
          readiness === "ready" ? (
            <a href={`/programs/evidence/${item.id}/download`} style={{ color: "var(--accent)" }}>
